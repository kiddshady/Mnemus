...
const parche = await store.saveSettings({ ultimaVista: 'mazos' });
ok('guardar un parche no pisa el resto', parche.ultimaVista === 'mazos' && parche.nuevasPorDia === store.DEFAULT_SETTINGS.nuevasPorDia);
// Una clave nueva del código tiene que aparecer en un archivo viejo.
await store.writeJSON(store.SETTINGS_FILE, { schema: store.SCHEMA, ultimaVista: 'inicio' });
const completado = await store.loadSettings();
ok('las claves nuevas se completan solas', 'nuevasPorDia' in completado && completado.ultimaVista === 'inicio');
